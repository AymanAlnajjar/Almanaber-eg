...
import { FiChevronDown, FiCalendar, FiUser, FiChevronRight, FiChevronLeft } from 'react-icons/fi';
...
const newsCategories = [
  { value: 'all', label: 'all_news' },
  { value: 'project_completion', label: 'project_completion' },
  { value: 'project_development', label: 'project_development' },
  { value: 'project_milestone', label: 'project_milestone' },
  { value: 'company_achievement', label: 'company_achievement' },
  { value: 'industry_insights', label: 'industry_insights' },
];
    ...
    const fetchNews = async () => {
      try {
        const url = selectedCategory === 'all'
          ...
          : `${process.env.NEXT_PUBLIC_API_URL}/news?category=${selectedCategory}`;
              ...
              >
                <span className="text-gray-700">
                  {selectedCategory === 'all' 
                    ? t('news_page', 'select_category')
                    : t('news_page', newsCategories.find(c => c.value === selectedCategory)?.label || '')
                  }
                </span>
                <FiChevronDown className={`text-gray-500 transition-transform ${showCategoryDropdown ? 'rotate-180' : ''}`} />
              </button>
              ...
              {newsItems.map((news: NewsItem) => (
                ...
                >
                  ...
                  <div className="relative h-48 md:h-56 lg:h-64 2xl:h-72 4k:h-80">
                    <Image
                      src={news.main_image || '/placeholder.svg'}
                  ...
                  <div className="p-6">
                    {/* Meta Information */}
                    <div className={`flex items-center text-sm text-gray-500 mb-3 ${locale === 'ar' ? 'flex-row-reverse' : 'flex-row'}`}>
                    ...
                    {/* Title */}
                    <h3 className={`text-xl font-bold text-[#1a2950] mb-3 line-clamp-2 ${locale === 'ar' ? 'text-right' : 'text-left'}`}>
                      {locale === 'ar' ? news.title_ar : news.title_en}
                    </h3>

                    {/* Description */}
                    <p className={`text-gray-600 mb-4 line-clamp-3 ${locale === 'ar' ? 'text-right' : 'text-left'}`}>
